const btnAddTaskElm = document.querySelector("#btn-add-task");
const txtTaskElm = document.querySelector("#txt-task");
const listContainerElm = document.querySelector("#list-container");
const noItemsElm = document.querySelector("#list-container > div");
...
btnAddTaskElm.addEventListener('click', (e) => {
    ...
    if(!task){
        txtTaskElm.focus();
        txtTaskElm.select();//there is a api calls select() for input elements
        return;
    }
    ...
    iconElm.setAttribute("class", "bi bi-trash3 align-self-end me-2");
...
listContainerElm.addEventListener('click', (e) => {
    if(e.target.getAttribute("class") === 'bi bi-trash3 align-self-end me-2'){
...
listContainerElm.addEventListener('keydown', (e) => {
    switch (e.code){
        ...
        case "Delete":
            ...
            const iconElm = e.target.closest(".list-item").querySelector("i")
            iconElm.click();
